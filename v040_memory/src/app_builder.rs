...
pub async fn run_app(configuration: Configuration) -> anyhow::Result<()> {
    let mut voting_machine_init: VotingMachine = create_voting_machine(configuration);
    let mut memory_store = MemoryStore::new(voting_machine_init.clone()).await?;
    ...
    loop {
        ...
        let voting_machine: VotingMachine = memory_store.get_voting_machine().await?;
        ...
        match command {
            ...
            Some("voter") => {
                ...
                match voter_name_optn {
                    ...
                    Some(voter_name) => {
                        ...
                        let outcome = voting_machine_init.vote(paper);
            ...
            Some("votants") => {
                let voters = voting_machine_init.get_voters();
            ...
            Some("scores") => {
                let scores = voting_machine_init.get_scoreboard();
                println!("Scores: ");
        ...
        memory_store.put_working_machine(voting_machine).await?;
    }
